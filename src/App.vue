<template>
	...
	<router-view v-slot="{ Component }">
		<keep-alive>
			<component :is="Component" :leaders="leaders" :isLoading="isLoading"></component>
		</keep-alive>
	</router-view>
</template>
...
export default {
	...
	provide() {
		return {
			addDataToLeaders: this.addDataToLeaders,
            loadLeadersData: this.loadLeadersData
		};
	},
	data() {
		return {
			leaders: [{ id: "a", name: "max", score: 34 }],
            isLoading:true
		};
	},
	methods: {
		addDataToLeaders(enteredName, scored) {
			const now = new Date();
			const timeString = `${now.getFullYear()}/${now.getMonth()}/${now.getDate()} ${now.getHours()}:${now.getMinutes()}`;
			fetch(
				"https://color-blind-game-f8385-default-rtdb.firebaseio.com/leaders.json",
				{
					method: "POST",
					headers: {
		...
		loadLeadersData() {
			fetch(
				...
				.then((response) => {
					if (!response.ok) throw new Error("fetch 錯誤🤷‍♂️");
